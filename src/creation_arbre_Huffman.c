...
void enfiler(struct File *file, uint8_t nouvelle_profondeur, struct arbre* nouvel_arbre) /* inspiré d'un exemple sur internet */
{
...
struct Element* defiler(struct File *file) /* inspiré d'un exemple sur internet */
{
...
struct arbre* creation_arbre(uint8_t* profondeurs, uint8_t* valeurs) {
    ...
    arbre->valeur=0;
    arbre->gauche = arbre_vide();       /*On part d'un arbre avec deux fils qui sont des arbres vides*/
    ...
    premier_element_gauche->arbre=arbre->gauche;  /*On met le premier fils gauche dans la file*/
    ...
    while (file->premier != NULL) {
        ...
        if (profondeur_courante < profondeur_arbre && profondeurs[profondeur_courante-1] == 0) {
            arbre_courant-> gauche = arbre_vide();
            arbre_courant-> droit = arbre_vide();
            enfiler(file, profondeur_courante+1, arbre_courant->gauche); /* parcours en largeur */
            enfiler(file, profondeur_courante+1, arbre_courant->droit);
        }

        else if (profondeurs[profondeur_courante-1] != 0) {    /* -1 car il n'y a pas de valeurs prévues pour la profondeur 0 */
...
uint8_t decode_Huffman_DC(uint8_t indiceDC, struct header *header, struct extended_FILE *stream) {
...
uint8_t decode_Huffman_AC(uint8_t indiceAC, struct header *header, struct extended_FILE *stream) {
