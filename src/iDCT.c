...
*/
uint8_t conversion_spatiale(int16_t** matrice_frequences, uint8_t x, uint8_t y)
{
    uint8_t n = 8;
    ...
    for (uint8_t lambda = 0; lambda < 8; lambda++) {
        for (uint8_t mu = 0; mu < 8; mu++) {
            ...
            coeff_spatial += calcule_C(lambda)*calcule_C(mu)*first_cos*second_cos*matrice_frequences[lambda][mu];
        }
    }
