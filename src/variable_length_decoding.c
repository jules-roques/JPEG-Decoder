...
uint16_t* decode_bloc(uint16_t predicateurDC, uint8_t indiceDC, uint8_t indiceAC, struct header *header, struct extended_FILE *stream)
{
    uint16_t* vecteur64 = malloc(64 * sizeof(uint16_t));

    uint8_t magnitudeDC = decode_Huffman_DC(indiceDC, header, stream);
    vecteur64[0] = decodeDPCM_DC(predicateurDC, magnitudeDC, stream);             /*premiere case (avec DC)*/
