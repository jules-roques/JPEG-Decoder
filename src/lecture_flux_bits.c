...
/* --------------------------------------------- STRUCTURES POUR LA LECTURE DANS LE STREAM -------------------------------------------- */
...
*/
void free_byte_buffer(struct byte_buffer *byte_buffer)
{
    free(byte_buffer);
}
...
*/
struct extended_FILE *open_file(const char *file_name, const char *oppening_mode)
{
...
void close_file(struct extended_FILE *stream)
{
    ...
    if (enf_of_file != 0xffd9) {
        print_warning(stdout, "Le marqueur de fin de flux JPEG 0xffd9 n'est pas reconnu: 0x%x\n", enf_of_file);
    }
...
{   
    ...
    uint8_t bit_to_read = (stream->byte_buffer->byte >> stream->byte_buffer->bit_to_read_index) & 1;
