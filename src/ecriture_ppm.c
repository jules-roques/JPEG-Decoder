...
void ecrit_entete_ppm(struct header *header, FILE* fichier)
{
    ...
    fprintf(fichier, "%s\n%d %d\n%d\n", valeur_magique, largeur, hauteur, valeur_maximale);
}
...
void ecrit_corps_ppm(struct header *header, struct extended_FILE* stream, FILE *fichier_ppm)
{
    ...
    for (uint16_t i = 0; i < nb_mcu_par_colonne - 1; i++) {
        ...
        /* dernier MCU */
        extrait_nouveau_MCU(mcu, header, stream);
        change_width_to_write(largeur_mcus_derniere_colonne, mcu);  /* écrit avec la largeur spécifique des blocs de dernière colonne */
        ecrit_MCU(mcu, largeur_image, fichier_ppm);
    ...
    change_height_to_write(hauteur_mcus_derniere_ligne, mcu);  /* écrit avec la hauteur spécifique des blocs de dernière ligne */
    for (uint16_t j = 0; j < nb_mcu_par_ligne - 1; j++) {
        extrait_nouveau_MCU(mcu, header, stream);
        ecrit_MCU(mcu, largeur_image, fichier_ppm);
        free_partiel_MCU(mcu);
