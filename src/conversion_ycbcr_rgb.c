...
/*
    Convertit 3 matrices Y, Cb, et Cr de même dimensions en une matrice RGB de uint32
*/
uint32_t** YCbCr_to_RGB(uint32_t** matrice_Y, uint32_t** matrice_Cb, uint32_t** matrice_Cr, uint8_t hauteur, uint8_t largeur)
{
    ...
    for(uint8_t i = 0; i < hauteur; i++) {
        ...
        for (uint8_t j = 0; j < largeur; j++) {
            matrice_RGB[i][j] = conversion((int16_t) matrice_Y[i][j], (int16_t) matrice_Cb[i][j], (int16_t) matrice_Cr[i][j]);
        }
    }
