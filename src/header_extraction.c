...
/* ------------------------------- STRUCTURES ET FONCTIONS ASSOCEES (extraction de l'en-tête, affichage, free) ------------------------ */
/*
...
- La structure
- Sa fonction d'extracion de l'en-tête (prenant soin de gérer les situation non autorisées par la norme en levant des erreurs)
- Son free
...
struct APPO_section *extract_APPO_section(struct extended_FILE *stream)
{
    ...
    if (strcmp(appo->jfif, "JFIF") != 0) {
        print_error(stderr, "Le format d'encapsulation attendu (JFIF) n'est pas le bon: %s\n", appo->jfif);
        return NULL;
    }

    if (appo->X != 1 || appo->Y != 1) {
        print_error(stderr, "La version du format d'encapsulation JFIF attendue (1.1) n'est pas la bonne: %u.%u\n", appo->X, appo->Y);
        return NULL;
    }
...
struct DHT_section* extract_DHT_section(struct extended_FILE *stream)
{
    ...
    if (zeros != 0) {
        print_error(stderr, "les 3 bits avant le type de table de Huffman dans le flux doivent être à 0\n");
        return NULL;
    }
    if (dht->indice > 3) {
        print_error(stderr, "L'indice de la table de Huffman doit être entre 0 et 3 compris. Indice trouvé: %u\n", dht->indice);
        return NULL;
    }
...
void print_DHT_section(struct DHT_section *dht)
{
    ...
    affiche_tab_uint8("                ", 40, dht->tableau_profondeurs, (uint16_t) dht->nb_profondeurs);
    printf("        Symboles (%u):\n", dht->nb_symboles);
    affiche_tab_uint8("                ", 40, dht->tableau_symboles, dht->nb_symboles);
}




/* ------------------------------------------ SECTION DQT (Tables de quantification) ----------------------------------------------- */
...
struct DQT_section *extract_DQT_section(struct extended_FILE *stream)
{
    ...
    if (dqt->indice_table_quantif > 3) {
        print_error(stderr, "Indice de table de quantification doit être entre 0 et 3 compris. Trouvé: %u\n", dqt->indice_table_quantif);
        return NULL;
    }
    if (dqt->indice_table_quantif > 1) {
        print_error(stderr, "Notre programme ne supporte pas correctement de tables de quantification d'indice de plus de 2. Trouvé: %u\n", dqt->indice_table_quantif);
        return NULL;
    }
...
void print_DQT_section(struct DQT_section *dqt)
{
    ...
    affiche_tab_uint16("                ", 40, dqt->table_quantif, (uint16_t) dqt->nb_elems_table_quantif);
}




/* --------------------------------------------------- SECTION SOFO (Start of frame) ------------------------------------------------ */

/* --- SOUS SECTION (une pour chaque comporante, N composantes, cf sujet) --- */

struct SOFO_sub_section {
    uint8_t id_composante;             /* 1 pour Y, 2 pour Cb et 3 pour Cr (seulement pour notre décodeur) */
...
struct SOFO_sub_section *extract_SOFO_sub_section(struct extended_FILE *stream)
{
    ...
    if (sub_sofo->facteur_ech_horizontal < 1 || sub_sofo->facteur_ech_horizontal > 4) {
        print_error(stderr, " Composante d'identifiant %u: "
    ...
    if (sub_sofo->facteur_ech_vertical < 1 || sub_sofo->facteur_ech_vertical > 4) {
        print_error(stderr, " Composante d'identifiant %u: "
    ...
    if (sub_sofo->ind_table_quantif > 3) {
        print_error(stderr, " Composante d'identifiant %u: "
                            "l'indice de table de quantification associée doit être entre 0 et 3 compris."
...
void print_SOFO_sub_section(char* alinea, uint8_t indice_appartition, struct SOFO_sub_section *sub_sofo)
{
    printf("%sComposante %s\n", alinea, nom_composante(indice_appartition));
    printf("%s        id %u\n", alinea, sub_sofo->id_composante);
    printf("%s        facteurs d'échantillonage (hxv) %ux%u\n", alinea, sub_sofo->facteur_ech_horizontal, sub_sofo->facteur_ech_vertical);
    printf("%s        table quantification indice %u\n", alinea, sub_sofo->ind_table_quantif);
}
...
struct SOFO_section {
    ...
    struct SOFO_sub_section **sous_sections;   /* ordre des composantes est toujours le même : Y puis Cb puis Cr*/
};
...
struct SOFO_section *extract_SOFO_section(struct extended_FILE *stream)
{
    ...
    sofo->sous_sections = malloc(sofo->nb_composantes * sizeof(struct SOFO_sub_section*));
    for (uint8_t i = 0; i < sofo->nb_composantes; i++) {
        sofo->sous_sections[i] = extract_SOFO_sub_section(stream);
        if (sofo->sous_sections[i] == NULL) {
            print_error(stderr, "Erreur lors de l'excration s'une sous-section SOFO\n");
            return NULL;
        }
        if (sofo->sous_sections[0]->facteur_ech_horizontal % sofo->sous_sections[i]->facteur_ech_horizontal != 0) {
            print_error(stderr, "Le facteur d'échantillonage horizontal de la composante %s ne divise pas parfaitement celui de Y. "
                                "(%u ne divise pas %u)\n", nom_composante(i), sofo->sous_sections[i]->facteur_ech_horizontal, sofo->sous_sections[0]->facteur_ech_horizontal);
            return NULL;
        }
        if (sofo->sous_sections[0]->facteur_ech_vertical % sofo->sous_sections[i]->facteur_ech_vertical != 0) {
            print_error(stderr, "Le facteur d'échantillonage vertical de la composante %s ne divise pas parfaitement celui de Y. "
                                "(%u ne divise pas %u)\n", nom_composante(i), sofo->sous_sections[i]->facteur_ech_vertical, sofo->sous_sections[0]->facteur_ech_vertical);
            return NULL;
        }
        somme_de_produits += sofo->sous_sections[i]->facteur_ech_horizontal * sofo->sous_sections[i]->facteur_ech_vertical;
    }

    if (somme_de_produits > 10) {
        print_error(stderr, "La somme des produits hxv doit être inférieure ou égale à 10. On trouve %u\n", somme_de_produits);
        return NULL;
    }
...
void free_SOS_sub_section(struct SOS_sub_section *sub_sos)
{
    free(sub_sos);
}
...
struct SOS_section *extract_SOS_section(struct extended_FILE *stream)
{
    ...
    sos->sous_sections = malloc(sos->nb_composantes * sizeof(struct SOS_sub_section*));
    ...
    if (sos->Ss != 0 || sos->Se != 63) {
        print_error(stderr, "Les indices de la sélection spectrale trouvés (%u,%u) "
                        "ne sont pas conformes au mode baseline (qui attend les indices 0,63)\n", sos->Ss, sos->Se);
        return NULL;
    }

    if (sos->Ah != 0 || sos->Al != 0) {
        print_error(stderr, "ERREUR: Les informations d'approximation des coeffs AC pour le mode progressif "
...
/* --------------------------------------------- EN-TETE (RASSEMBLEMENT DES SECTIONS) -------------------------------------------------- */
/*
...
struct header {
    ...
    struct SOS_section *sos; /* (Il y a plusieurs SOS dans le décodeur progressif) */
    ...
    struct arbre ***matrice_arbres;  /* organisé en matrice: le type de matrice pour les ligne et l'incide de matrice pour les colonnes */
...
struct header *extract_header(struct extended_FILE *stream)
{
    ...
    if (marqueur != 0xffd8) {
        print_error(stderr, "Le marqueur de début de section 0xffd8 n'est pas reconnu: 0x%x\n", marqueur);
        return NULL;
    }
    ...
    do {
        ...
        switch (marqueur) {
            ...
            case 0xffc4: /* DHT */
                header->dht_sections = realloc(header->dht_sections, (header->nb_dht + 1) * sizeof(struct DHT_section*));
            ...
            case 0xffdb: /* DQT */
                header->dqt_sections = realloc(header->dqt_sections, (header->nb_dqt + 1) * sizeof(struct DQT_section*));
            ...
            case 0xfffe: /* COM */
                header->com_sections = realloc(header->com_sections, (header->nb_com + 1) * sizeof(struct COM_section*));
            ...
            default:
                print_error(stderr, "Marqueur de début de section non reconnu dans le fichier jpeg: 0x%x\n", marqueur);
                return NULL;
        }
    } while (marqueur != 0xffda); /* Une fois que l'on a lu la section S0S on s'arrête */
    
    /* Tri des du tablau header->sections_dqt, pour que l'indice de la table de quantif soit le même que sa position dans le tableau */
    struct DQT_section **dqt_sections_triees = malloc(header->nb_dqt * sizeof(struct DQT_section*));
    for (uint8_t i = 0; i < header->nb_dqt; i++) {
        dqt_sections_triees[header->dqt_sections[i]->indice_table_quantif] = header->dqt_sections[i];
    }
    ...
    /* Remplissage de la matrice d'arbres de Huffman (normalement de taille 2x2)
        ...
        - les indices de la matrice correnspondes auc indices de la table de Huffman
            les lignes pour l'indice i_H
            les colonnes pour l'indice i_AC/DC
    ...
    */
    ...
    for (uint8_t i = 0; i < header->nb_dht; i++) {
        ...
        header->matrice_arbres[ligne][colonne] = creation_arbre(copy_array_uint8(profondeurs, nb_profondeurs), symboles);
    }
...
/* -------------------------------------------------------------- FONCTIONS POUR RECUPERER INFOS ----------------------------------------------------------- */
...
*/
struct arbre *get_arbre_huffman(uint8_t type, uint8_t indice, struct header *header)
{
    if (type > 1) {
        print_error(stderr, "Le type doit être inférieur ou égal à 1. %u passé en argument\n", type);
        return NULL;
    }
    if (indice > 1) {
        print_error(stderr, "L'indice doit être inférieur ou égal à 1. %u passé en argument\n", indice);
        return NULL;
    }
...
/*
    Fonction pour récupérer une table de quantification associée à une composante (à son identifiant)
*/
uint16_t *get_table_quantif(uint8_t identifiant_composante, struct header *header)
{
    uint8_t indice_table_quantif;
    for (uint8_t i = 0; i < header->sofo->nb_composantes; i++) {
        if (identifiant_composante == header->sofo->sous_sections[i]->id_composante) {
            indice_table_quantif = header->sofo->sous_sections[i]->ind_table_quantif;
        }
    }
    uint16_t *table_quantif = header->dqt_sections[indice_table_quantif]->table_quantif;
    uint8_t taille_table_quantif = header->dqt_sections[indice_table_quantif]->nb_elems_table_quantif;
...
/*
    ...
    de la _indice_appartition_composante_ ième composante apparaissant dans le MCU
*/
uint8_t get_huffman_index_DC(uint8_t indice_appartition_composante, struct header *header)
{
    return header->sos->sous_sections[indice_appartition_composante]->ind_table_huffman_DC;
}
...
/*
    ...
    de la _indice_appartition_composante_ ième composante apparaissant dans le MCU
*/
uint8_t get_huffman_index_AC(uint8_t indice_appartition_composante, struct header *header)
{
    return header->sos->sous_sections[indice_appartition_composante]->ind_table_huffman_AC;
}
...
const char *get_magic_value(struct header *header)
{
    ...
    print_error(stderr, "Nombre de composantes non reconnu. On attend soit 1 pour des niveaux de gris, soit 3 pour une imagee RGB. Cependant on trouve %u\n", header->sofo->nb_composantes);
    return NULL;
}
...
*/
uint8_t get_id_composante(uint8_t indice_apparition_composante, struct header *header)
{
    return header->sos->sous_sections[indice_apparition_composante]->id_composante;
}
...
uint8_t get_precision_image(struct header *header)
{
    if (header->sofo->precision != 8) {
        print_error(stderr, "On attend une précision de 8 bits en mode Baseline. On trouve  %u\n", header->sofo->precision);
        return -1;
    }
...
*/
uint8_t get_horizontal_sampling_factor(uint8_t id_composante, struct header *header)
{
    for (uint8_t i = 0; i < header->sofo->nb_composantes; i++) {
...
*/
uint8_t get_vertical_sampling_factor(uint8_t id_composante, struct header *header)
{
    for (uint8_t i = 0; i < header->sofo->nb_composantes; i++) {
        if (id_composante == header->sofo->sous_sections[i]->id_composante) {
